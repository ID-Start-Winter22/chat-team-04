...
# Returns Python dict instead of JSON                   #
#                                                       #
#                                                       #
...
def handle_route(start: str, destination: str):
    """
    Calculates time between start and destination

    Arguments:
        start: Station you start your journey at
        destination: Station that you want to leave the train/bus
    Returns:
        A dict that looks like the following:
             {
                "from": ...,
                "to": ...,
                "time_needed": ...
    ...
    if travel_time:
        ...
        return {"error": "could not calculate travel time for those stations!"}

# Test
# result = handle_route("Holzapfelkreuth", "Poccistr")
# print(result)